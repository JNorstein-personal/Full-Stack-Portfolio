...
The project has completed substantial requirements, architecture, RSVP-system, routing, content, wireframe, and visual-design planning and is transitioning into implementation.



The current development phase focuses on establishing the application foundation, reusable design system, shared site shell, routing architecture, centralized event configuration, client/server boundary, and an early Ubuntu deployment proof.



The complete RSVP backend and guest-facing RSVP workflow are specified but are not yet fully implemented.
...
All public pages are intended to use a common application shell containing:
...
The application is being designed so that essential functionality does not depend on hover behavior.
...
Planned shared style responsibilities include:
...
Reusable component families are planned for common interaction patterns, including:
...
Guest-facing wedding information is intended to be centralized rather than duplicated across React page components.



Centralized application data will include information such as:
...
Pages that depend upon venue or schedule configuration are intended to consume the same centralized configuration so that changing the active event arrangement does not require manually editing multiple pages.
...
VITE\_API\_BASE\_PATH=/wedding/api
...
Windows `node\_modules` directories are not transferred to the server.
...
The present workstream establishes the durable application foundation required for later feature development.
...
The workstream is complete only when the application foundation has been exercised as an actual navigable application and its production deployment architecture has been proven.
